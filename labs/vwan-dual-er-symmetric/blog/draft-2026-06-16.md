# Three blind spots in the ExpressRoute DR guide that vWAN, route maps, and cloud peers introduced
...
**Thesis:** The ExpressRoute DR guide is still a good foundation, but secured Virtual WAN, vWAN route maps, and cloud-side route selection add three failure modes the older CE-router-centric model does not cover.
...
That is not theory. In this lab, a dual-ExpressRoute, dual-hub design with two Megaport MCRs and a single GCP Cloud Router produced asymmetric, hash-dependent TCP failures. The baseline showed roughly 54 cross-contaminating flows per firewall. vWAN route maps reduced that contamination by 98 percent in the active/active design, and the active/passive design gave clean failover in 54.2 seconds. But neither design fully solved steady-state symmetry for the spoke `/24` prefixes, because GCP Cloud Router kept both paths installed at equal priority despite very different AS-path lengths.

That last sentence is the real finding. AS-path prepend worked in the BGP table, but the cloud peer's route installation model still defeated the expected forwarding outcome.
...
But C1 is not a full fix. The same `02-cr-routes-summary.txt` shows every spoke `/24` still installed through both MCRs. GCP Cloud Router kept both routes in `bestRoutes` even when one path was two hops and the other was five or seven hops. The data plane saw the consequence: `08-probe-spoke3-vma.txt` had only two TCP successes out of five, with three timeouts.

So the honest C1 verdict is: route maps made the design much better, but GCP's installed route model left residual `/24` ECMP.
...
Again, that table is both good news and bad news. The good news is that the BGP intent is obvious. The bad news is that both `/24` paths remain installed at priority 0, so steady-state ECMP remains possible.

The data plane and firewall logs confirm the partial result. `04-probe-spoke1-vma.txt` had two successes out of five. `06-probe-spoke3-vma.txt` had five successes out of five, which shows the hash-dependent nature of the problem. The KQL summary in `08-azfw-kql-cross-contamination-summary.txt` says:
...
So C2 is not a 100 percent steady-state symmetry fix against this GCP peer. But the failover behavior is excellent.
...
That is the C2 value proposition: partial steady-state symmetry, clean active/passive failover.

## 9. The central finding: route selection at the cloud peer can defeat AS-path prepend

The climax of the lab is not that vWAN Route Maps work. They do. The climax is that working AS-path prepend was still insufficient.

For the `/23` aggregates, GCP installed one path. For all four spoke `/24`s, GCP installed both paths at `priority=0`, even when one path was much longer:
...
The evidence is in C1 `02-cr-routes-summary.txt`, C2 `02-gcp-cr-bestroutes-table.txt`, and the C2 verdict `15-verdict.md`. Niobe's verdict states the key behavior plainly: GCP Cloud Router derives each VPC dynamic route's priority from MED, not AS-path length, so unequal AS-paths can still become equal-priority VPC routes. Equal priority means ECMP candidates.

That is the blind spot the older article could not have anticipated. The article is built around CE routers and BGP path selection. In a cloud peer, the BGP best-path view and the installed VPC route priority view are not the same thing. You can make the BGP path look worse and still have the cloud data plane install it as an equal-cost route.

## 10. What the article still gets right

The original recommendations still matter.

More-specific route advertisement is still the most deterministic path-selection tool. In this lab, the `/23` aggregates were single-path precisely because they were not cross-propagated between hubs. Longest-prefix behavior is still king, but the more-specific `/24`s won over the clean `/23`s and reintroduced ECMP.
...
ECMP guidance is also still relevant. The surprise is not that ECMP exists. The surprise is where it reappears: after a successful AS-path prepend, at the cloud VPC route-priority layer.

## 11. What actually fixes it: Mech C3

C1 and C2 are valuable, but neither is the final answer for a peer that installs unequal AS paths as equal-priority `/24` VPC routes.

The full-symmetry fix is **Mech C3**:

1. Suppress the more-specific `/24` advertisements on the standby circuit, so GCP sees only the `/23` aggregate from that side during normal operation.
2. Or apply a GCP-side MED or route-priority signal so standby `/24`s are not equal-priority ECMP candidates.

The first option uses the behavior the lab already proved: `/23` aggregates were single-path in both C1 and C2. The second option works with GCP's actual installed-route model instead of assuming AS-path length is enough.

The practical recommendation is therefore:

| Goal | Recommendation |
|---|---|
| Modernize the Learn Scenario 2 active/active pattern inside vWAN | Use C1, outbound route maps with AS 64496, but do not claim full `/24` symmetry against GCP without suppressing standby specifics or setting GCP-side priority |
| Modernize the Learn Scenario 1 active/passive pattern inside vWAN | Use C2 if clean failover is the main goal; the lab proved 54.2 second failover and 45.4 second restore |
| Achieve 100 percent steady-state symmetry for GCP-to-Azure `/24`s | Add C3: suppress standby `/24`s or set GCP-side MED or route priority |
...
2. Confirm the four spoke `/24`s are ECMP in `gcloud compute routers get-status`.
...
| C1 residual `/24` ECMP and spoke3 2/5 data-plane result | `show-output\design-c-mechC1-symmetric-2026-06-16\02-cr-routes-summary.txt`, `08-probe-spoke3-vma.txt`, `14-verdict.md` |
| C2 AS 64496 five times, partial steady state | `show-output\design-c-mechC2-symmetric-2026-06-16\02-gcp-cr-bestroutes-table.txt`, `08-azfw-kql-cross-contamination-summary.txt`, `15-verdict.md` |
...
2. **Before and after GCP route table:** baseline equal two-hop `/24` paths, C1/C2 prepended path visible with 64496, but both still priority 0. Anchor it to C1 `02-cr-routes-summary.txt` and C2 `02-gcp-cr-bestroutes-table.txt`.
