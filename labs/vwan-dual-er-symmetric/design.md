...
> ⚠️ **GCP /24-ECMP caveat (discovered in Mech C1 evidence, 2026-06-16).** The "wins" column above describes BGP *best-path* selection. It does **not** guarantee single-path forwarding on GCP. The GCP Cloud Router derives each VPC dynamic-route's priority from MED, **not** AS-path length, so it installs **both** the 2-hop and the 7-hop path for a given /24 at `priority=0`: i.e. it ECMPs them despite the unequal AS-path. Only the /23 supernets (10.10.0.0/23, 10.20.0.0/23) resolve to a single path. Therefore C2's blanket prepend forces the *intent* (MCR1 primary) but, exactly like C1, leaves residual /24 ECMP on the GCP→Azure return path. Full /24 determinism requires suppressing the more-specific /24 advertisements on the standby circuit (advertise only the /23 aggregate): tracked as **Mech C3**: or a GCP-side MED/priority lever. See `show-output/design-c-mechC1-symmetric-2026-06-16/14-verdict.md`.
